...
library.ConfigSystem = function(self, mainFolder, subFolder, name)
    local userFolder = game.Players.LocalPlayer.UserId
    local baseFolder = "KarmaPanda"
    local configFolder = baseFolder .. "/" .. mainFolder .. "/" .. subFolder
    ...
    if not isfolder(baseFolder .. "/" .. mainFolder) then
        makefolder(baseFolder)
    end
    ...
    -- Ensure the config file path includes the .txt extension
    local configFilePath = configFolder .. "/" .. name .. ".txt"

    -- Save default settings if the config file doesn't exist
    if not isfile(configFilePath) then
        writefile(configFilePath, httpservice:JSONEncode(DefaultSettings))
        print("Default settings saved to " .. configFilePath)
    end

    local configSystem = {}
    configSystem.configFilePath = configFilePath

    configSystem.Load = function(self)
        local success, content = pcall(function()
            return readfile(self.configFilePath)
        end)
        ...
        if success then
            ...
            print("Failed to load config: " .. content)
        end
    end
    ...
    configSystem.Save = function(self)
        ...
        writefile(self.configFilePath, httpservice:JSONEncode(config))
    end
...
library.CreateWatermark = function(self, name)
	...
	watermark.text = " " .. name:gsub("{game}", gamename):gsub("{fps}", "0FPS") .. " ";
	...
	runservice.Heartbeat:Connect(function()
		...
		if not name:find("{fps}") then
			watermark.label.Text = " " .. name:gsub("{game}", gamename):gsub("{fps}", "0FPS") .. " ";
		end
	...
	window.CreateTab = function(self, name)
		...
		tab.CreateSector = function(self, name, side)
			...
			sector.AddToggle = function(self, text, default, callback, flag)
				...
				toggle.AddMiniGuiLabel = function(self, default, callback, flag)
					local minilabel = {}
				
					minilabel.Gui = Instance.new("ScreenGui")
					minilabel.Gui.Name = "MiniGuiLabel"
					minilabel.Gui.Parent = game.CoreGui
				
					minilabel.Frame = Instance.new("Frame")
					minilabel.Frame.Name = "LabelFrame"
					minilabel.Frame.Size = UDim2.new(0, 200, 0, 50)
					minilabel.Frame.Position = UDim2.new(0, 130, 0, 0)
					minilabel.Frame.BackgroundTransparency = 0.5
					minilabel.Frame.BackgroundColor3 = Color3.new(0, 0, 0)
					minilabel.Frame.Active = true
					minilabel.Frame.Draggable = true
					minilabel.Frame.Parent = minilabel.Gui
				
					minilabel.Main = Instance.new("TextLabel")
					minilabel.Main.Name = "Label"
					minilabel.Main.BackgroundTransparency = 1
					minilabel.Main.Size = UDim2.new(1, 0, 1, 0)
					minilabel.Main.Position = UDim2.new(0, 0, 0, 0)
					minilabel.Main.ZIndex = 4
					minilabel.Main.Font = window.theme.font
					minilabel.Main.Text = text
					minilabel.Main.TextColor3 = window.theme.itemscolor
					minilabel.Main.TextSize = 15
					minilabel.Main.TextStrokeTransparency = 1
					minilabel.Main.TextXAlignment = Enum.TextXAlignment.Left
					minilabel.Main.Parent = minilabel.Frame
					...
					miniArrowButton.Parent = minilabel.Frame
				
					local isOpen = true
				
					local function toggleGUI()
						isOpen = not isOpen
						minilabel.Frame.Visible = isOpen
						miniArrowButton.Text = isOpen and "v" or "^"
					end
				
					miniArrowButton.MouseButton1Click:Connect(toggleGUI)
				
					if rainbow == true then
						RainbowText(minilabel.Main)
					end
				
					updateevent.Event:Connect(function(theme)
						minilabel.Main.Font = theme.font
						minilabel.Main.TextColor3 = theme.itemscolor
					end)
				
					minilabel.Set = function(self, value)
						minilabel.Main.Text = value
					end
...
loadstring(game:HttpGet("https://raw.githubusercontent.com/Jeikaru/Roblox/main/Notification"))();
return library;
